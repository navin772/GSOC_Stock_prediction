...
side = st.sidebar.selectbox("Selcect one", ["Dashboard", "Foreign Markets", "Stock News", "Fbprophet"])
...
location_index = {'India': '^NSEI', 'USA': '^DJI', 'Germany': '^GDAXI', 'UK': '^FTSE', 'China': '^HSI', 'Japan': '^N225'}
top_stocks = {'^NSEI': ['INFY.NS', 'RELIANCE.NS', 'BAJFINANCE.NS', 'TCS.NS', 'SBIN.NS'], '^DJI': ['AAPL', 'GOOG', 'TSLA', 'MSFT', 'UNH'], '^GDAXI': ['AIR.DE', 'VOW3.DE', 'ADS.DE', 'DTE.DE', 'BMW.DE']}
...
if side == "Dashboard":
    ...
    st.sidebar.markdown("### The Dashboard will show the predictions for the top 5 stocks for short/long term. You can also get predictions for any other stock using the next block")
    
    text = '{location}'.format(location=location)
    st.markdown("### See predictions for top stocks for your country "+text)
    st.markdown("###### *Not your country? Select your country from the dropdown below*")
